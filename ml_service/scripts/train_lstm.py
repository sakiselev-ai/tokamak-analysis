...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train bi-LSTM+Attention model for disruption prediction"
    )
    parser.add_argument(
        "--data",
        type=str,
...
def compute_metrics(
    model: LSTMAttentionModel, X_test: np.ndarray, y_test: np.ndarray
) -> dict:
...
def train(args: argparse.Namespace) -> dict:
    ...
    model = LSTMAttentionModel(hyperparams=hyperparams)
    ...
    def progress_callback(
        ...
        if epoch % 5 == 0:
            ckpt_path = checkpoint_dir / f"lstm_epoch_{epoch:03d}.pt"
    ...
    if "history" in all_metrics["train_metrics"]:
        all_metrics["train_metrics"] = {
            k: v
            for k, v in all_metrics["train_metrics"].items()
            if k != "history"
        }
