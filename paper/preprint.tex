...
outperforming the TokaMark Group~1 baseline (NRMSE = 0.163).
...
models on a time-series forecasting task following the TokaMark
benchmark protocol~\citep{tokamark2026}.
Using 100 MAST shots with normalized features, we train LSTM and
Transformer forecasting variants to predict future plasma parameter
trajectories and measure performance using Normalized Root Mean
Squared Error (NRMSE).

The LSTM Forecaster achieves NRMSE = 0.143, outperforming
...
providing a strong foundation for future work on plasma state
prediction and disruption time-to-event estimation.
...
(3) competitive forecasting results, with our LSTM Forecaster
achieving NRMSE = 0.143, surpassing the TokaMark Group~1
baseline (NRMSE = 0.163);
