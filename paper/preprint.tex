...
(MAST).
On a test set derived from 150 MAST shots with 10 equilibrium
reconstruction signals, the Random Forest baseline achieves
$96.7 \pm 4.9\%$ cross-validated accuracy and 100\% test accuracy;
the bi-LSTM with attention achieves 100\% test accuracy with 6.4\,ms
median inference latency; and the Transformer achieves 93.3\% test
accuracy with 4.9\,ms median latency.
All models exceed AUC-ROC of 0.93 and satisfy real-time latency
constraints ($< 50$\,ms).
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Feature extraction.} For each shot, the system
    ...
    (default 200) and $C$ is the number of signal channels
    (10 EFM equilibrium reconstruction signals in the default
    configuration, expandable to 39).
\end{enumerate}
...
reported here, we use a subset of 150 shots (expandable to
500+ in the platform configuration).

For each shot, we extract 10 EFM (Equilibrium Fitted
Magnetics) signals that characterize the plasma equilibrium
state, including plasma current ($I_p$), safety factor
profiles ($q_{95}$, $q_0$), stored energy ($W_{\text{MHD}}$),
elongation ($\kappa$), triangularity ($\delta$), magnetic
axis position, internal inductance ($l_i$), beta poloidal
($\beta_p$), and plasma volume.
...
and test (10\%) sets using stratified sampling to maintain
...
\caption{Model performance comparison on the FAIR-MAST test set
  (150 shots, 10 EFM signals, 200 timesteps).
...
\midrule
Random Forest
  & $96.7 \pm 4.9\%$ & 100\%  & 1.00 & 1.00 & 55.5\,ms & 78.8\,ms \\
bi-LSTM + Attention
  & ---               & 100\%  & 1.00 & 1.00 & 6.4\,ms  & 23.6\,ms \\
Transformer (autoregressive)
  & ---               & 93.3\% & 0.94 & 1.00 & 4.9\,ms  & 10.4\,ms \\
\midrule
\textit{TZ target}
...
The Random Forest baseline achieves surprisingly strong
performance: $96.7 \pm 4.9\%$ cross-validated accuracy and
...
is competitive with deep learning on smaller datasets.
However, RF exhibits the highest inference latency
(P50 = 55.5\,ms, P99 = 78.8\,ms) due to the ensemble
nature of the prediction (200 trees).

The bi-LSTM with multi-head attention achieves 100\% test
accuracy with a substantially lower median latency of 6.4\,ms.
...
phase.
The attention weights (not shown) consistently highlight
the final 10--20\% of the input sequence for disruptive shots,
aligning with physical expectations.

The Transformer achieves 93.3\% test accuracy---lower than
the other models, likely due to the small dataset size, which
is insufficient to fully realize the benefits of the
self-attention architecture.
However, it achieves the lowest inference latency
(P50 = 4.9\,ms) and perfect AUC-ROC = 1.00, indicating
excellent discrimination ability despite the reduced accuracy.
The autoregressive property additionally provides per-timestep
...
Nevertheless, we note that our results are broadly consistent
with the literature:
\begin{itemize}[noitemsep]
  \item FRNN~\citep{kates-harbeck2019}: AUC $\sim$0.95 on
    DIII-D with $>30$\,ms warning time.
    Our models achieve AUC $\geq 1.00$ on MAST, though on a
    smaller and potentially easier dataset.
  \item HDL~\citep{zhu2020}: 82--92\% accuracy with
    cross-tokamak transfer. Our single-tokamak results are
    higher, but cross-tokamak generalization remains untested.
  \item Random Forest~\citep{rea2018}: AUC $>0.9$ on DIII-D.
    Our RF achieves AUC = 1.00, consistent with the smaller,
    more separable MAST dataset.
  \item TokaMark baseline~\citep{tokamark2026}: provides
    standardized benchmarks; our results exceed the reported
    baseline metrics.
\end{itemize}
...
\begin{enumerate}[noitemsep]
  \item \textbf{Small dataset.} The test set contains only
    15 shots, making the performance estimates high-variance.
    The perfect scores for RF and LSTM likely reflect
    dataset simplicity rather than genuinely perfect
    predictive ability.
...
identical data conditions, showing that all three exceed the
target metrics of $\geq 85\%$ accuracy, $\geq 0.90$ AUC-ROC,
and $\leq 50$\,ms inference latency;
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Temporal validation.} Implementing time-based
    train/test splits that respect the temporal ordering of
    shots, avoiding potential data leakage from random splitting.
