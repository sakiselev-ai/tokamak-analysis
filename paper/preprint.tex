...
98.4\% cross-validated accuracy (AUC-ROC = 0.983);
the Transformer achieves 87.8\% accuracy (AUC-ROC = 0.938); and
the bi-LSTM with attention achieves 78.4\% accuracy
(AUC-ROC = 0.867) in quick-training mode.
Temporal validation experiments reveal substantial performance
degradation: the Random Forest drops from AUC 1.0 (random split)
to 0.969 (temporal), while the Transformer drops from 1.0 to 0.892,
...
\caption{Model performance comparison on the FAIR-MAST dataset
  (500 shots, 20 plasma parameters, 200 timesteps).
  CV Accuracy is 3-fold stratified cross-validation.
  Test metrics are on a held-out 20\% split.
  Latency is P99, measured on a 2-vCPU configuration.}
\label{tab:results}
...
\midrule
Random Forest
  & $98.4 \pm 0.8\%$ & $0.983 \pm 0.012$ & 100\% & 1.000 & 73.0\,ms \\
bi-LSTM + Attention
  & $78.4 \pm 0.9\%$ & $0.867 \pm 0.013$ & 80.0\% & 0.944 & 39.4\,ms \\
Transformer
  & $87.8 \pm 1.0\%$ & $0.938 \pm 0.034$ & 100\% & 1.000 & 131.0\,ms \\
\midrule
\textit{TZ target}
...
The Random Forest baseline achieves the highest cross-validated
accuracy: $98.4 \pm 0.8\%$ with AUC-ROC $= 0.983 \pm 0.012$.
The perfect test-set scores (100\% accuracy, AUC 1.0) suggest
overfitting to the random split, which is confirmed by the
temporal validation results below.
...
The bi-LSTM with multi-head attention achieves $78.4\%$
cross-validated accuracy and AUC-ROC $= 0.867$ in
quick-training mode (5 epochs).
The relatively lower performance reflects the limited
training budget; with full training (50 epochs with early stopping),
we expect significant improvement, as the bidirectional
processing and attention mechanism require more epochs to
converge on the temporal patterns preceding disruptions.
The P99 inference latency of 39.4\,ms meets the 50\,ms
real-time constraint.

The Transformer achieves $87.8\%$ cross-validated accuracy
with AUC-ROC $= 0.938$, outperforming the LSTM despite
also using a limited training budget.
The self-attention mechanism appears to converge faster
than recurrent architectures on this dataset.
However, the P99 latency of 131\,ms exceeds the real-time
constraint, suggesting that model compression or
sequence length reduction would be necessary for
...
Random splits produce inflated metrics (100\% accuracy for RF
...
shots $\geq 12035$), all models show significant accuracy
degradation (25--39\%), while AUC-ROC drops are more moderate
(3--27\%).
The Random Forest shows the best temporal robustness
(AUC 0.969), while the bi-LSTM suffers the largest drop
(AUC 0.671), likely due to insufficient training epochs
in quick mode.
...
\midrule
\textbf{Ours (RF, CV)}         & MAST   & \textbf{0.983} & --- \\
Ours (Transformer, CV)          & MAST   & 0.938 & $\geq 30$\,ms \\
Ours (bi-LSTM, CV)              & MAST   & 0.867 & $\geq 30$\,ms \\
Ours (RF, temporal)             & MAST   & 0.969 & --- \\
\bottomrule
\end{tabular}
\end{table}

Our Random Forest achieves the highest cross-validated
AUC-ROC = 0.983, exceeding prior work on conventional tokamaks.
...
class imbalance (86.8\% disruptions) may inflate RF metrics.
Under temporal validation, RF maintains AUC = 0.969,
demonstrating genuine predictive capability.
The Transformer (CV AUC = 0.938) is competitive with
FRNN~\citep{kates-harbeck2019} (AUC = 0.94)
despite using fewer training epochs.
The bi-LSTM underperforms in quick-training mode (AUC = 0.867)
but is expected to improve with full training.
